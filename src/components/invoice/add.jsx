  ...
  const AddInvoice = () => {
    ...
    const handleBuyerChange = (buyerId) => {
      const buyer = scopedBuyers.find((entry) => entry._id === buyerId);

      setInvoice((prev) => {
        const isStandardRateScenario = prev.scenarioId === "SN001" || prev.scenarioId === "SN002";
        const scenarioId = isStandardRateScenario
          ? buyer?.registrationType === "Unregistered"
            ? "SN002"
            : "SN001"
          : prev.scenarioId;

        return { ...prev, buyerId, scenarioId };
      });
    };
    ...
    const handleProductDescriptionChange = (value) => {
      setInvoice((prev) => ({
        ...
        items: prev.items.map((item, index) =>
          index === activeItemIndex ? { ...item, productDescription: value } : item
        ),
      }));
    };
    ...
    const removeItem = (index) => {
      if (invoice.items.length > 1) {
        ...
        setActiveItemIndex((current) => {
          if (current === index) return Math.min(index, filtered.length - 1);
    ...
    const addNewItem = () => {
      setActiveItemIndex(invoice.items.length);
      ...
      const fbrPayload = {
        ...
        items: invoice.items.map((itm) => {
          ...
          return {
            ...
            rate: formatTaxRate(itm.taxRate),
            uoM: itm.uoM || "",
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase">
                  Product Description (Item {activeItemIndex + 1})
                </label>
                <input
                  type="text"
                  list="product-description-options"
                  required
                  value={invoice.items[activeItemIndex]?.productDescription || ""}
                  onChange={(e) => handleProductDescriptionChange(e.target.value)}
                  className="w-full mt-1 p-2 lg:p-3 border rounded-lg text-sm"
                  placeholder="Select or enter product description"
                />
                <datalist id="product-description-options">
                  {productDescriptionOptions.map((description) => (
                ...
                <tbody className="divide-y">
                  {invoice.items.map((itm, idx) => (
                    <tr
                      ...
                      className={activeItemIndex === idx ? "bg-blue-50" : "hover:bg-slate-50"}
                    >
